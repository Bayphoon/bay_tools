...
describe('PersonalDataManager', () => {
  ...
  it('copies only portable personal data and does not rewrite an unchanged snapshot', async () => {
    ...
    await write(root, 'Doc/bookmarks/index.json', '{"items":[{"title":"常用工具"}]}')
    await write(root, 'Doc/bookmarks/icons/tool.png', 'bookmark icon')
    ...
    expect(await readFile(join(root, 'UserData/alice/bookmarks/index.json'), 'utf8')).toContain('常用工具')
    expect(await readFile(join(root, 'UserData/alice/bookmarks/icons/tool.png'), 'utf8')).toBe('bookmark icon')
