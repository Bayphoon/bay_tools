...
describe('BookmarksPage', () => {
  ...
  it('reads a clipboard image as a custom icon', async () => {
    const user = userEvent.setup()
    const image = new Blob(['clipboard-png'], { type: 'image/png' })
    const read = vi.fn(async () => [{ types: ['image/png'], getType: vi.fn(async () => image) } as unknown as ClipboardItem])
    Object.defineProperty(navigator, 'clipboard', { configurable: true, value: { read, writeText: vi.fn().mockResolvedValue(undefined) } })
    Object.defineProperty(URL, 'createObjectURL', { configurable: true, value: vi.fn(() => 'blob:bookmark-icon') })
    Object.defineProperty(URL, 'revokeObjectURL', { configurable: true, value: vi.fn() })

    render(<BookmarksPage />)
    await screen.findByText('普通链接')
    await user.click(screen.getByRole('button', { name: '新增书签' }))
    await user.click(screen.getByRole('button', { name: '自定义图标' }))
    await user.click(screen.getByRole('button', { name: '读取剪贴板图片' }))
    await waitFor(() => expect(read).toHaveBeenCalledTimes(1))
    expect(await screen.findByText(/clipboard-icon-\d+\.png/)).toBeTruthy()
