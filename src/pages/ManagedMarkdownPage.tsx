...
export function ManagedMarkdownPage({ documentId }: { documentId: string }) {
  ...
  return <div className="page full-height-page markdown-page">
    ...
    <div className={`markdown-workspace mode-${mode}`}>
      {mode !== 'preview' && <div className="markdown-editor"><Editor height="100%" language="markdown" value={content} onChange={(value) => { setContent(value ?? ''); setDirty((value ?? '') !== document.content || document.title !== (useAppStore.getState().managedMarkdown?.documents.find((item) => item.id === document.id)?.title ?? document.title)); setStatus('idle') }} theme="vs-dark" options={{ minimap: { enabled: false }, fontSize: 14, fontFamily: 'Cascadia Code, Consolas, monospace', wordWrap: 'on', automaticLayout: true, scrollBeyondLastLine: false }} /></div>}
